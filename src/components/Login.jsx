...
//     const { useLign, loginUser, loginData, logoutUser, loading } = useLogin();
...
"use client";
import { useState } from "react";
import { Mail, Lock, LayoutGrid, Zap, TrendingUp, CheckCircle } from "lucide-react";
import Link from "next/link";
import { useRouter } from "next/navigation";
import { useLogin } from "@/hooks/useLoginUser";
    ...
    <main className="mt-10 bg-gray-50 text-gray-800 min-h-screen flex items-center justify-center p-6">
      <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
        ...
        <div className="px-4 lg:px-8 flex flex-col justify-center gap-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-1 gap-4 w-full">
            {features.map((f) => (
              <div key={f.title} className="bg-white p-4 sm:p-5 rounded-xl flex items-start gap-4 border border-gray-100 shadow-sm hover:shadow-md transition">
        ...
        <div className="flex items-center justify-center px-4 lg:px-8">
          <div className="w-full max-w-lg bg-white rounded-3xl shadow-2xl border border-gray-100 p-8 sm:p-10">
            ...
            {/* ROLE SELECTION */}
            <fieldset className="pt-2 mb-4">
              <legend className="block text-sm font-medium text-gray-500 mb-2">Select your role</legend>
                ...
                {roles.map((r) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-2">
                      {role === r.id ? <CheckCircle className="h-5 w-5 text-green-600" /> : <span className="h-5 w-5 border-2 border-gray-300 rounded-full" />}
                      <span>{r.label}</span>
                    </div>
                  </label>
                ))}
