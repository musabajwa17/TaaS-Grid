...
// export default function EnrichStudentResume() {
...
//   const suggestions = enrichedData ? (enrichedData).suggestions : undefined;
//   useEffect(() => {
...
export default function EnrichStudentResume() {
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-6 bg-white rounded-2xl shadow-md border border-gray-100 my-5">
      ...
      <p className="text-gray-600 text-center mt-2">
        Select your preferences below to generate a tailored and professional CV
        enhancement.
      </p>
          ...
          <div className="space-y-6">
            {suggestions?.summary_improvement && (
              <SuggestionBlock
                ...
                content={suggestions.summary_improvement}
              />
            )}

            {Array.isArray(suggestions?.missing_sections) &&
              suggestions.missing_sections.length > 0 && (
                <SuggestionList
                  title="📂 Missing Sections"
                  items={suggestions.missing_sections}
                />
              )}

            {Array.isArray(suggestions?.missing_details) &&
              suggestions.missing_details.length > 0 && (
                <SuggestionList
                  title="⚠️ Missing Details"
                  items={suggestions.missing_details}
                />
              )}

            {Array.isArray(suggestions?.suggested_additions) &&
              suggestions.suggested_additions.length > 0 && (
                <SuggestionList
                  title="💡 Suggested Additions"
                  items={suggestions.suggested_additions}
                />
              )}
            ...
            {suggestions?.tone_recommendation && (
              <SuggestionBlock
                ...
                content={suggestions.tone_recommendation}
              />
            )}
          </div>
        </div>
...
function SuggestionBlock({ title, content }) {
  return (
    <div className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm">
